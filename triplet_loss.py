...
def triplet_loss(model, batch, M):
    # model.train()
    #M = 1 

    #emb = model(batch["X"].cuda())
    #emb = emb.view(emb.size(0),-1)
    # emb = model.extract_features(batch["X"].cuda())
    emb = batch["X"].cuda()
    y = batch["y"].cuda()
    if len(emb) == 0:
        print('no sample for triplet loss')
        return 0, 0
    # print('triplet_loss emb: {}, y: {}'.format(emb.size(), y.size()))
    
    with torch.no_grad():
        triplets = get_triplets(emb, y, M)
    #print(triplets.size())
    ...
    # ap_D = (f_A - f_P).pow(1).sum(1)  # .pow(.5)
    # an_D = (f_A - f_N).pow(1).sum(1)  # .pow(.5)
    #losses = F.relu(ap_D - an_D + 1.)
    losses = F.relu(ap_D - an_D + M)
    # losses = F.relu(ap_D - an_D + 2.)

    return losses.mean(), triplets.size(0)
...
def get_triplets(embeddings, y, margin=1):
